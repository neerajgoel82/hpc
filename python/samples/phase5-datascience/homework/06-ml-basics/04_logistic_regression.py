"""
Homework: 04_logistic_regression.py

Complete the exercises below based on the concepts from the classwork file.

Instructions:
1. Read the corresponding classwork file first.
2. Implement each exercise_* function below.
3. Run: python3 04_logistic_regression.py
4. Uncomment the exercise calls in main() to test.
"""

# Exercise 1: Implement Binary classification (two classes).
def exercise_1():
    """Implement Binary classification (two classes)."""
    pass  # Your code here

# Exercise 2: Implement Multiclass classification (more than two).
def exercise_2():
    """Implement Multiclass classification (more than two)."""
    pass  # Your code here

# Exercise 3: Implement Probability predictions.
def exercise_3():
    """Implement Probability predictions."""
    pass  # Your code here

# Exercise 4: Implement Decision boundaries.
def exercise_4():
    """Implement Decision boundaries."""
    pass  # Your code here


if __name__ == "__main__":
    print("Homework: 04_logistic_regression")
    exercise_1()  # Uncomment and implement above
    exercise_2()  # Uncomment and implement above
    exercise_3()  # Uncomment and implement above
    exercise_4()  # Uncomment and implement above
