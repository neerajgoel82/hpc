"""
Homework: 02_seaborn_statistical.py

Complete the exercises below based on the concepts from the classwork file.

Instructions:
1. Read the corresponding classwork file first.
2. Implement each exercise_* function below.
3. Run: python3 02_seaborn_statistical.py
4. Uncomment the exercise calls in main() to test.
"""

# Exercise 1: Implement Distribution plots (histograms, KDE).
def exercise_1():
    """Implement Distribution plots (histograms, KDE)."""
    pass  # Your code here

# Exercise 2: Implement Box plots.
def exercise_2():
    """Implement Box plots."""
    pass  # Your code here

# Exercise 3: Implement Violin plots.
def exercise_3():
    """Implement Violin plots."""
    pass  # Your code here

# Exercise 4: Implement Pair plots.
def exercise_4():
    """Implement Pair plots."""
    pass  # Your code here


if __name__ == "__main__":
    print("Homework: 02_seaborn_statistical")
    exercise_1()  # Uncomment and implement above
    exercise_2()  # Uncomment and implement above
    exercise_3()  # Uncomment and implement above
    exercise_4()  # Uncomment and implement above
